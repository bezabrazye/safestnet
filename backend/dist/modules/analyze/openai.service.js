...
let OpenAiService = class OpenAiService {
    ...
    async analyzeUrlSecurity(url, signals, locale = 'en') {
        const systemPrompts = {
        ...
        const system = systemPrompts[locale] || systemPrompts.en;
...
`;
    ...
    formatFactors(factors, locale) {
        return factors.map(f => `${f.name}: ${f.value} (weight: ${f.weight}) [${f.evidence}]`).join('\n');
    }
    generateSources(signals) {
        const sources = [];
        if (signals.gsb) {
            sources.push({
    ...
    async analyzeFactCheck(text, image, locale = 'en') {
        const systemPrompts = {
...
`;
    ...
    async analyzeDataCheck(dto, locale = 'en') {
        const systemPrompts = {
...
`;
    ...
    async analyzeFraudDetection(text, images, locale = 'en') {
        const systemPrompts = {
