...
let AnalyzeService = class AnalyzeService {
    constructor(signals, scoring, openai, multimodalAnalyzer, storage) {
        this.signals = signals;
        this.scoring = scoring;
        this.openai = openai;
        this.multimodalAnalyzer = multimodalAnalyzer;
        this.storage = storage;
        this.i18n = new i18n_service_1.I18nService();
    }
    async handleUrl(dto) {
    ...
    async saveFeedback(id, verdict, ip) {
        let result = InMemory.get(id);
        if (!result) {
            const fromDb = await this.storage.getAnalysisById(id);
            if (!fromDb) {
        ...
        result.feedbacks.push({ verdict, createdAt: new Date().toISOString() });
        InMemory.set(id, result);
    ...
    async handleMultimodalAnalysis(dto) {
        ...
        try {
            ...
            const result = {
                ...
                category: categoryMap[analysis.status] || 'Medium',
                confidence: Math.round(analysis.confidence * 100),
                summary: analysis.summary,
                explanation: {
                    factors: analysis.reasons.map((r) => ({
                        name: r.code,
                    ...
                    riskFactors: analysis.reasons.map((r) => r.code),
    ...
    generateSecurityFactors(signals, locale, url) {
        const factors = [];
        const t = this.i18n.getTranslations(locale);
        // Domain Age
        ...
        if (signals.page?.suspiciousPatterns !== undefined) {
            ...
            const keywords = Array.isArray(signals.page.suspiciousKeywords) ? signals.page.suspiciousKeywords : [];
    ...
    detectExcessiveTracking(url) {
        ...
        if (trackingCount >= 5)
            return 0.8; // Много трекинга
        if (trackingCount >= 3)
            return 0.5; // Средний трекинг
        if (trackingCount >= 1)
            return 0.3; // Немного трекинга
        return 0;
    }
    listTrackingParams(url) {
        try {
            const u = new URL(url);
    ...
    detectSuspiciousRegistrar(registrar) {
        if (!registrar)
            return 0;
...
exports.AnalyzeService = AnalyzeService = __decorate([
    ...
    __metadata("design:paramtypes", [signals_service_1.SignalsService,
        ...
        multimodal_analyzer_service_1.MultimodalAnalyzerService,
