...
export type AnalyzeInput = {
  url?: string;
  text?: string;
  images?: string[];       // image URLs or data URIs (base64)
  videoFrames?: string[];  // treat as images
  locale?: string;         // e.g., "ru", "lv", "en-US"
...
@Injectable()
export class MultimodalAnalyzerService {
  ...
  private buildUserContent(input: AnalyzeInput) {
    const content: any[] = [];
    ...
    if (input.comment) parts.push(`USER COMMENT:\n${input.comment}`);
  ...
  async analyzeSubmission(input: AnalyzeInput) {
    try {
  ...
  private normalizeReport(report: any, input: AnalyzeInput) {
    return {
  ...
  private getFallbackReport(input: AnalyzeInput) {
    return {
      status: 'suspicious',
      risk_score: 50,
  ...
  private getFallbackRecommendations(locale?: string) {
    ...
    return recommendations[locale as keyof typeof recommendations] || recommendations.ru;
  }

  private generateInputSummary(input: AnalyzeInput): string {
    const parts = [];
    if (input.url) parts.push('URL');
    if (input.text) parts.push('текст');
    if (input.images?.length) parts.push(`${input.images.length} изображений`);
    if (input.videoFrames?.length) parts.push(`${input.videoFrames.length} кадров видео`);
    if (input.comment) parts.push('комментарий пользователя');
