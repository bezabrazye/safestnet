export async function fetchUrlWithMeta(url: string) {
  try {
    ...
    const suspiciousMap: Array<{name: string; regex: RegExp}> = [
    ...
    for (const item of suspiciousMap) {
      if (item.regex.test(html)) suspiciousKeywords.push(item.name);
    }
